...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for anode in self.a_levels[level-1] :
            for s in anode.effnodes :
                self.s_levels[level].add(s)

        for anode in self.a_levels[level-1] :
            for s in self.s_levels[level] :
                if s in anode.effnodes:
                    s.parents.add(anode)
                    anode.children.add(s)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1 in node_a1.action.effect_add :
            if a1 in node_a2.action.effect_rem :
                return True

        for a1 in node_a1.action.effect_rem :
            if a1 in node_a2.action.effect_add :
                return True

        for a2 in node_a2.action.effect_add :
            if a2 in node_a1.action.effect_rem :
                return True

        for a2 in node_a2.action.effect_rem :
            if a2 in node_a1.action.effect_add :
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a in node_a1.action.precond_pos:
            if a in node_a2.action.precond_neg :
                return True

        for a in node_a1.action.precond_neg:
            if a in node_a2.action.precond_pos :
                return True

        for a in node_a2.action.precond_pos:
            if a in node_a1.action.precond_neg :
                return True

        for a in node_a2.action.precond_neg:
            if a in node_a1.action.precond_pos :
                return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        def negate_node(node: PgNode_s) -> PgNode_s :
            return PgNode_s(node.symbol, not(node.is_pos))

        return node_s1 == negate_node(node_s2)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        if bool(node_s1.parents.intersection(node_s2.parents)) :
            return False
